	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			...
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll('SELECT ID, job, job_grade, posao, loadout, position, phone_number, firstname, lastname, firma, status, permission_level FROM `users` WHERE `identifier` = @identifier', {
					['@identifier'] = player.getIdentifier()
				}, function(result)
					...
					userData.perm = result[1].permission_level
...
ESX.RegisterServerCallback('esx:spawnVehicle', function(playerId, cb, model, coords, heading)
	...
	--SetEntityDistanceCullingRadius(entityHandle, 9999999999999999.9)--fix?
...
AddEventHandler('esx:setPerm', function(id, perm)
	local xPlayer = ESX.GetPlayerFromId(id)
	if xPlayer then
		xPlayer.setPerm(perm)
	end
end)
