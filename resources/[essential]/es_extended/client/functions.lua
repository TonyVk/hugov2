...
ESX.ShowNotification = function(msg, ms)
	local sec = 3000
	if ms then
		sec = ms
	end
...
ESX.ShowPBar = function(msg, ms, cb)
	...
	if cb then
		Citizen.SetTimeout(sec + 100, cb)
	end
end

ESX.ShowAdvancedNotification = function(title, subject, msg, icon, iconType)
	DisplayHud(true)
	AddTextEntry('sxAdvancedNotification', msg)
	SetNotificationTextEntry('sxAdvancedNotification')
...
ESX.Game.SpawnVehicle = function(modelName, coords, heading, cb, vehicleProperties)
	...
	ESX.TriggerServerCallback('esx:spawnVehicle', function(entityNetworkId)
		...
		if ESX.Game.RequestNetworkControlOfEntity(entityHandle) then
			SetVehicleOnGroundProperly(entityHandle)
			local tablice = GetVehicleNumberPlateText(entityHandle)
			...
			SetVehicleIsStolen(entityHandle, false)
			SetVehicleIsWanted(entityHandle, false)
			SetVehicleAutoRepairDisabled(entityHandle, true)
			SetModelAsNoLongerNeeded(model)
			TriggerServerEvent('garage:addKeys', tablice)
...
AddEventHandler('esx:showPBar', function(msg, ms)
	ESX.ShowPBar(msg, ms)
end)
