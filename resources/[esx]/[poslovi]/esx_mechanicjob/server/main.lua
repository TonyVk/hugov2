...
TriggerEvent('esx_society:registerSociety', 'mechanic', 'mechanic', 3, 3, 3, {type = 'private'})
...
RegisterServerEvent('meh:UzmiSarafciger')
AddEventHandler('meh:UzmiSarafciger', function()
	local src = source
	...
	if xPlayer ~= nil then
		...
		if job.name == "mechanic" then
			if xPlayer.getMoney() >= 500 then
				if Sarafcigeri > 0 then
					local item = xPlayer.getInventoryItem('sarafciger')
					if item.limit ~= -1 and (item.count + 1) > item.limit then
						xPlayer.showNotification("Ne stane vam vise u inventory!")
					else
						xPlayer.removeMoney(500)
						xPlayer.addInventoryItem('sarafciger', 1)
						Sarafcigeri = Sarafcigeri-1
					end
				end
			end
		end
