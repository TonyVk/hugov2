...
RegisterCommand("prison", function(src, args, raw)
	if args[1] ~= nil and args[2] ~= nil and args[3] ~= nil then
		ExecuteCommand("jail "..args[1].." "..args[2].." "..args[3])
	else
		ExecuteCommand("jail")
	end
end)
...
function JailLogin()
	...
	ESX.ShowNotification("Bili ste u zatvoru kada ste izasli, zato ste sad vraceni u zatvor!")
...
function UnJail()
	...
	ESX.ShowNotification("Pusteni ste iz zatvora, pridrzavajte se zakona!")
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			ESX.ShowNotification("Imate " .. jailTime .. " minuta preostalo u zatvoru!")
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Paket"

					if not v["state"] then
						PackageText = "Vec odnesen"
					end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Vec ste odnjeli ovaj paket!")
							end
...
function PackPackage(packageId)
	...
	local PackageObject = CreateObject(GetHashKey(model), Package["x"], Package["y"], Package["z"], true)
	...
	while Packaging do
		...
		if not IsPedUsingScenario(PlayerPedId(), "PROP_HUMAN_BUM_BIN") then
			...
			ESX.ShowNotification("Prekinuto!")
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Pakiranje... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			if DistanceCheck <= 2.0 then
				if IsControlJustPressed(0, 38) then
					...
					TriggerServerEvent("zatvor:radutijeku")
				end
			end
		end
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			elements = {
				...
				{ label = "Oslobodi osobu", value = "unjail_player" }
			}
		}, 
	function(data, menu)
		...
		if action == "jail_closest_player" then
			...
			ESX.UI.Menu.Open(
          		...
          		{
            		title = "Vrijeme u zatvoru (minute)"
          		},
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Vrijeme mora biti u minutama!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Morate staviti neki razlog!")
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									TriggerServerEvent('esx_qalle_brottsregister:add', GetPlayerServerId(closestPlayer), reason)
								end
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Vas zatvor je prazan!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Zatvorenik: " .. playerArray[i].name .. " | Vrijeme u zatvoru: " .. playerArray[i].jailTime .. " minuta", value = playerArray[i].identifier })
				end
