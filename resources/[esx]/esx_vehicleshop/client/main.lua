...
function SpawnNpcove()
	local pedmodel = GetHashKey("cs_siemonyetarian")
	LoadModel(pedmodel)
	PosaoPed = CreatePed(0, pedmodel, Config.Zones.ShopEntering.Pos, Config.Zones.ShopEntering.Heading, false, true)
	...
	exports.qtarget:AddEntityZone("salon", PosaoPed, 
	{
		name="autosalon",
		debugPoly=false,
		useZ = true
		...
		options = {
			{
				event = "salon:OtvoriSalon",
				icon = "fa fa-car",
				label = "Otvori salon menu"
			},
		},
		distance = 2.5
	})
...
RegisterNetEvent('salon:OtvoriSalon')
AddEventHandler('salon:OtvoriSalon', function()
	local koord = GetEntityCoords(PlayerPedId())
	if #(koord-Config.Zones.ShopEntering.Pos) <= 2.5 then
