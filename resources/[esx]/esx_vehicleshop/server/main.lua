...
AddEventHandler('ugovor:prodajtuljanu2', function(target, plate, price, nid)
	local _target = target
	local _source = source
	TriggerClientEvent('esx_contract:PoslaoMu', _target, 1, plate, price, _source, nid)
end)

ESX.RegisterServerCallback('ugovor:prodajtuljanu3', function(source, cb, target, plate, price)
	local _source = target
	local xPlayer = ESX.GetPlayerFromId(_source)
	local _target = source
	local tPlayer = ESX.GetPlayerFromId(_target)
	if tPlayer.getMoney() >= price then
		MySQL.Async.fetchAll('SELECT owner FROM owned_vehicles WHERE owner = @identifier AND plate = @plate', {
...
RegisterServerEvent('esx_contract:showAnim')
AddEventHandler('esx_contract:showAnim', function(target, plate)
	TriggerClientEvent('esx_contract:showAnim2', target, source, plate)
end)

RegisterServerEvent('esx_contract:PosaljiPoruke')
AddEventHandler('esx_contract:PosaljiPoruke', function(target, plate)
	local _source = source
	local _target = target
	...
	TriggerClientEvent("contract:ZamjenaVozila", _source)
end)
