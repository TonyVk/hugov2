...
function SpawnCpove()
	...
	for k,v in pairs(Config.Zones) do
		for i = 1, #v.Pos, 1 do
			table.insert(Cpovi, {ID = check, k = k, Items = v.Items, Koord = v.Pos[i], Spawnan = false, r = Config.Color.r, g =  Config.Color.g, b = Config.Color.b})
		end
	end
end
			...
			for i=1, #Cpovi, 1 do
				if Cpovi[i] ~= nil and Cpovi[i].Spawnan then
					if #(coords-Cpovi[i].Koord) < 1.5 then
						isInMarker  = true
						ShopItems   = Cpovi[i].Items
						currentZone = Cpovi[i].k
						LastZone    = Cpovi[i].k
						ID = i
						break
					end
