	...
	MySQL.Async.fetchAll(
      ...
      function(result)
        for i=1, #result, 1 do
			...
			table.insert(Vrata, {objekt = nil, ime = result[i].ime, koord = kord, posao = result[i].posao, dist = result[i].dist, lock = 4, model = tonumber(result[i].model)})
        end
...
AddEventHandler('vrata:DodajVrata', function(ime, coord, posao, dist, model)
	table.insert(Vrata, {objekt = nil, ime = ime, koord = vector3(coord.x, coord.y, coord.z), posao = posao, dist = dist, lock = 4, model = tonumber(model)})
