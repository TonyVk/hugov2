...
CreateThread(function()
	...
	if Config.Framework == 'ESX' then
		...
		PosaoCheck = function(job)
			if type(job) == 'table' then
				job = job[ESX.PlayerData.posao.name]
				if job and ESX.PlayerData.posao.grade >= job then
					return true
				end
			elseif job == ESX.PlayerData.posao.name or job == 'all' then
				return true
			end
			return false
		end
		...
		RegisterNetEvent('esx:setPosao', function(job)
	...
	elseif Config.Framework == 'QB' then
		...
		RegisterNetEvent('QBCore:Client:OnPosaoUpdate', function(JobInfo)
			PlayerData.posao = JobInfo
		end)
	...
	function CheckOptions(data, entity, distance)
		...
		if data.posao and not PosaoCheck(data.posao) then return false end
