...
Citizen.CreateThread(function()
	...
	while true do Wait(0)
		if IsControlJustReleased(1, Config.GUI.Key) then
			if not Pritiso then
				local Ped = PlayerPedId() 
				if Check(Ped) then SoundPlay("Open") SetCursorLocation(Config.GUI.Position.x, Config.GUI.Position.y) Pritiso = true end
			else
				local Ped = PlayerPedId() 
				if Check(Ped) then SoundPlay("Close") Pritiso = false end
			end
