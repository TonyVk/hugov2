    ...
    if perm == 1 then
        ...
        for i=1, #Config.Houses, 1 do
            if Config.Houses[i] ~= nil then
                table.insert(elements, {label = "Kuca "..Config.Houses[i].ID, value = Config.Houses[i].ID})
            end
        end

        ESX.UI.Menu.Open(
            'default', GetCurrentResourceName(), 'ukuce',
            {
                title    = "Izaberite kucu",
                align    = 'top-left',
                elements = elements,
            },
            function(data, menu)
                        ...
                        if mCijena == nil or tonumber(mCijena) < 0 then
                            ...
                            ESX.UI.Menu.Open(
                                ...
                                function(data2, menu2)
                                    ...
                                    TriggerServerEvent("kuce:NovaKuca", data2.current.value, korda, mCijena)
                                    menu2.close()
                                    menu.close()
                                    Wait(200)
