...
Config.deformationExponent = 0.6					-- How much should the handling file deformation setting be compressed toward 1.0. (Make cars more similar). A value of 1=no change. Lower values will compress more, values above 1 it will expand. Dont set to zero or negative.
...
Config.damageFactorEngine = 25.0					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
Config.damageFactorBody = 15.0					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
...
Config.degradingHealthSpeedFactor = 4				-- Speed of slowly degrading health, but not failure. Value of 10 means that it will take about 0.25 second per health point, so degradation from 800 to 305 will take about 2 minutes of clean driving. Higher values means faster degradation
...
Config.degradingFailureThreshold = 500.0			-- Below this value, slow health degradation will set in
Config.cascadingFailureThreshold = 250.0			-- Below this value, health cascading failure will set in
Config.engineSafeGuard = 150.0					-- Final failure value. Set it too high, and the vehicle won't smoke when disabled. Set too low, and the car will catch fire from a single bullet to the engine. At health 100 a typical car can take 3-4 bullets to the engine before catching fire.
