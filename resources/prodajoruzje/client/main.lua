local Keys = {
  ...
  RegisterCommand("smoke", function(source, args, raw)
	  ...
	  local exh = GetEntityBoneIndexByName(veh, "exhaust")
	  coords = GetWorldPositionOfEntityBone(veh, exh)
	  ...
	  while true do
		  Wait(50)
		  if GetVehicleCurrentRpm(veh) >= 0.70 then
			  Citizen.CreateThread(function()
				  coords = GetWorldPositionOfEntityBone(veh, exh)
				  UseParticleFxAssetNextCall(dict)
				  ...
				  table.insert(handleri, particleHandle)
				  if #handleri >= 10 then
					  StopParticleFxLooped(handleri[1], false)
					  table.remove(handleri, 1)
				  end
			  end)
		  else
			  if #handleri > 0 then
				  for i = 1, #handleri, 1 do
					  StopParticleFxLooped(handleri[i], false)
				  end
				  handleri = {}
			  end
		  end
  ...
  RegisterCommand("pop", function(source, args, raw)
	  local veh = GetVehiclePedIsIn(PlayerPedId())
	  EnableVehicleExhaustPops(veh, true)
  end)
  
  RegisterCommand("popoff", function(source, args, raw)
	  local veh = GetVehiclePedIsIn(PlayerPedId())
