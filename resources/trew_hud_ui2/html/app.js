...
window.onload = function () {
		...
		var eventCallback = {
			...
			setText: function(data) {
				...
				var html = data.value;
				if(data.id == "job"){
					key = document.querySelector('#'+data.id);
				}
			...
			updateVehicle: function(data) {
				...
				if (data.status == true) {
					...
					if(data.speed){
						...
						saferInnerHTML(document.querySelector('#vehicle-speed span'), 0);
					}
					...
					if ( data.seatbelt && (data.seatbelt.status == true) && (vehicleSeatbelt.classList.contains('on') == false) ) {
					...
					else if ( data.seatbelt && (data.seatbelt.status == false) && (vehicleSeatbelt.classList.contains('off') == false) ) {
