...
AddEventHandler('baseevents:enteredVehicle', function(currentVehicle, currentSeat, modelName, netId)
	...
	ESX.TriggerServerCallback('stage:ProvjeriVozilo',function(st)
		if st ~= 0 and st.servis ~= nil then
			Kvar = tonumber(st.servis)
			VrstaKvara = tonumber(st.kvar)
		end
	end, Tablica)
		...
		while UVozilu do
			...
			if ZabraniHud == false then
				if Sjedalo == -1 and Klasa ~= 13 and Klasa ~= 14 and Klasa ~= 15 and Klasa ~= 16 and Klasa ~= 17 and Klasa ~= 21 then
					...
					if Kilometri-zadnjikm >= 5 then
						zadnjikm = Kilometri
						TriggerServerEvent('vozilo:dodajKm', Tablica, Kilometri)
					end
					if Kvar ~= nil and Kilometri >= Kvar and not SapucoKvar then
						SapucoKvar = true
							...
							Citizen.CreateThread(function()
								while UVozilu do
									...
									if IsVehicleEngineStarting(vehica) then
										SetVehicleUndriveable(vehica, true)
									end
									Citizen.Wait(100)
								end
							end)
	...
	Citizen.CreateThread(function()
		while UVozilu do
			...
			if vehicleIsOn or Klasa == 21 then
				...
				if UVozilu and not vehicleIsOn then
					if Config.ui.showMinimap == false then
						DisplayRadar(false)
					end
...
local hRadio = false
RegisterCommand('+gtahud2', function()
...
RegisterKeyMapping('+gtahud2', 'Optimizacija GTA huda', 'keyboard', 'Q')
...
AddEventHandler("SaljiGear", function(gr)
	...
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		vehicleInfo = {
			...
			status = true,
			gear = gr,
			type = GetVehicleClass(GetVehiclePedIsIn(PlayerPedId())),
			km = round2(Kilometri, 0),
			speed = vehicleSpeed,
			seatbelt = { status = seatbeltIsOn },
			config = {
